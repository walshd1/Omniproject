...
export const ListProjectsResponseItem = zod.object({
  ...
  "omniInstanceId": zod.string().nullish().describe('OmniProject\'s backend-independent correlation GUID for this project (minted at creation). Drives programme membership, the closed-project registry and the on-close disposition flow.'),
  "status": zod.string().nullish().describe('Lifecycle status (backend-native); live reads exclude closed\/archived.'),
...
export const GetProjectIssuesResponseItem = zod.object({
  ...
  "riceScore": zod.number().nullish().describe('Reach × Impact × Confidence ÷ Effort prioritisation score (agile field group), surfaced for the portfolio prioritisation ranking. Many backends precompute this.'),
  "wsjf": zod.number().nullish().describe('Weighted Shortest Job First score (agile field group; SAFe), surfaced for the portfolio prioritisation ranking.'),
  "moscow": zod.string().nullish().describe('MoSCoW prioritisation bucket (agile field group). Free-form (e.g. must\/should\/could\/wont).'),
  "strategicContribution": zod.number().nullish().describe('How much this item contributes to its linked strategic goal(s), 0–100 (strategy field group).'),
  "strategicGoals": zod.array(zod.string()).optional().describe('The strategic goal(s) this item supports (strategy field group).'),
...
export const DeleteIssueResponse = zod.void()
...
export const CreateTaskItemResponse = zod.object({
  "id": zod.string(),
  "taskId": zod.string(),
  "kind": zod.enum(['issue', 'note']),
  "content": zod.string(),
...
export const CreateRaidEntryResponse = zod.object({
  "id": zod.string(),
  "projectId": zod.string(),
  "type": zod.enum(['risk', 'assumption', 'issue', 'dependency']),
  "title": zod.string(),
