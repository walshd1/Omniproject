...
describe("PortfolioIncome", () => {
  it("rolls projected vs invoiced up by programme", () => {
    renderWithProviders(<PortfolioIncome />, {
      client: seed([project({ id: "a", programmeId: "p1", programmeName: "Platform" })], { a: [issue({ id: "1", revenue: 1000, invoicedAmount: 600 })] }),
    });
    expect(screen.getByTestId("portfolio-income")).toBeInTheDocument();
    expect(screen.getByTestId("portfolio-income-row-p1")).toHaveTextContent("60%");
  });
