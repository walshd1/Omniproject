...
| **S9** | **In-CI container-image CVE scan — SHIPPED** (Trivy in the `docker-image` CI job, blocks on fixable HIGH/CRITICAL, `--ignore-unfixed`; installed checksum-verified via `fetch-verified.sh`). Remaining: gitleaks binary pin + extending mutation testing to crypto/auth. | Medium | Partially closed (CI) |
