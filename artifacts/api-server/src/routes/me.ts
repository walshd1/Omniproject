...
import { DEFAULT_USER_PREFS, getUserPrefs, hasUserPrefs, setUserPrefs } from "../lib/user-prefs";
...
router.get("/me/prefs", (req, res) => {
  ...
  if (!s) { res.json({ prefs: DEFAULT_USER_PREFS, stored: false }); return; }
  res.json({ prefs: getUserPrefs(s.sub), stored: hasUserPrefs(s.sub) });
});
