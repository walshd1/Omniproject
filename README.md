...
You keep several tabs open just to answer *"where do things stand"*: Jira for
engineering, a spreadsheet for budget, ServiceNow for the support queue, SAP for
the numbers finance trusts, and a status deck someone rebuilds by hand every
Friday. Nobody's going to migrate all of that into one tool, and you shouldn't
have to.
...
1. **Tool sprawl.** Engineering lives in Jira, finance trusts SAP, support runs
   ...
   covers. No single tool is "wrong," but no single person can see across all of
   them without spending a Friday afternoon copy-pasting into a status deck.
2. **Nobody trusts a second copy.** Every "unified" tool we looked at solves
   sprawl by importing your data into *itself*, and the moment it holds its own
   copy, that copy drifts from the real thing. Now someone has to answer "which
   number is true" and audit one more place data at rest can leak from.
3. **Migration is the blocker, not the idea.** People agree a single view across
   tools would help. Few can get their org to rip out Jira, SAP and ServiceNow
   to get it, because leadership won't accept the migration risk and the project
...
to keep in sync, and nothing new to secure. That's the whole bet. **[How
OmniProject works](#how-omniproject-works)** below shows how the build keeps
that promise structurally rather than by policy.
...
"Deploy on Railway" button comes next, once a maintainer has run and confirmed
that recipe. Full walkthrough, written for
...
answers both by design rather than by policy: it keeps no database of its own, so
...
contact with someone else's real Jira/SAP/OpenProject" stage. That only
happens when people other than the maintainer run it. If you'll
...
Most "single pane of glass" tools become a *second* place your data
...
brokered live to the systems that already own your data (Jira, OpenProject,
ServiceNow, SAP, … as illustrative examples, not a fixed list), through the
reference broker, n8n by default. There is no copy, so there is nothing
to fall out of sync. The backend is always right, and OmniProject renders it.

**Why no database helps:**
...
- **One less data store to secure and comply with.** No project data at rest in
...
**It's a layer that fits the workflow you already have.** Your tools, your
broker, your SSO stay exactly where they are;
...
or scaled out to **Kubernetes** in front of enterprise systems like **SAP**.
Because everything below the broker seam is pluggable, **a broker other than n8n
adapts rather than breaks.** Nobody has to move into a new app, migrate data,
...
**It's programme management, above the issue level.** You get a real delivery
picture, read through from your backends and shown only where a backend
supplies it:
...
- **Finance.** Earned Value (CPI/SPI), budget vs actuals, and **multi-currency**
  (each backend reports its own currency; convert to one display currency, and
...
- **DemoBroker proves the seam is generic.** A second, in-process broker
  ...
  with no backend at all, which is concrete proof that "above the seam knows
  nothing of n8n" holds in practice. n8n is the **reference broker** that ships for
...
- **CI enforces the boundary.** An **architecture-guard**
  ...
  **conformance suite** is the contract any adapter must pass: DemoBroker is the
  reference pass, the live n8n run is the real-world pass. The tests keep
  "swappable" true. See
...
- **The contract is published, versioned and generated from the code.** The full
  ...
  generated from `broker/{types,contract}.ts` in CI so they can't drift, and
...
OmniProject is **tool-agnostic**: it sits *above* whatever you run now
and whatever you move to later. In production the SPA and gateway ship as **one
container** (`omni-shell`) on port `3000`.

### Connect to (almost) anything

Anything n8n can reach becomes a backend, so the set of systems you can plug in
is open-ended, with no fixed connector list to wait on (and a different broker
...
- **No existing tool at all?** An **optional, customer-owned stateful database** (a
  ...
  teams with nothing to connect. It's the **non-preferred** path: the gateway stays stateless and the
...
- **Outbound, via webhook.** Push HMAC-signed OmniProject events to *any* endpoint:
  a SIEM, Slack, a customer system, or back into another n8n flow.
...
- **Methodology views.** One dataset rendered as Kanban, Scrum (burndown/velocity), Gantt/Waterfall, PRINCE2 stages, RAID log, or list; switch per team. The view layer is **open**, so you can add your own for a methodology we don't ship.
...
- **A broker-agnostic API.** The consumer-facing API lives *above* the swappable
  ...
  JSON discovery doc (`GET /api/discovery`), so you build against a stable contract
  rather than against n8n. (The *southbound* contract a broker implements is the separate
  `GET /api/contract`.)
...
Opt-in, gated historical replay against a **logging server you own**. It is
contract-complete and tested at the seam, but **unproven end-to-end** (demo mode
synthesises sample data; the n8n blueprint is a template). **Off by default**, admin-only,
...
- **Business rulesets.** An extra, PMO-configurable governance layer that is
  **restrict-only**: it runs *after* the hard gates and can only deny or warn,
  ...
  deletes) without becoming a way to escalate privilege.
...
untyped rows. It runs fail-soft, once, before anything is cached or derived. Repairs are
tallied and surfaced (a `Data repaired` badge + `X-OmniProject-Data-Repaired` header),
so a flaky backend degrades visibly rather than corrupting a roll-up in silence.
...
counts grow with nearly every merge, so see [docs/TESTING.md](docs/TESTING.md) for
...
axe at WCAG 2.1 A/AA). A live **n8n contract verification** and a
**load-test harness** (`pnpm --filter @workspace/scripts run load`) can drive
configurable concurrency against a real gateway → broker → backend path and fail
above a 1% error rate. The harness **has not yet been run for real**: it ships
...
the rest are off by default, and the whole product works without them. Point a
...
in a single step, with no separate widget-picking. Full walkthrough of what you
...
- **Explore** (`g e`). The modelling lab (snapshots, what-if, dependency links),
  kept separate from live data. See [docs/EXPLORATION.md](docs/EXPLORATION.md).
...
| `BROKER_TRACE_PAYLOADS` | gateway | **Dev only.** `1` additionally emits full (secret-scrubbed) request/response payloads in trace output, not just shapes. Requires `BROKER_TRACE=1`; inert in production. Use with care, since real payloads carry backend PII. |
| `BROKER_SPOOF` | gateway | **Dev only.** A vendor id (e.g. `openproject`) for the dev broker to present AS; it serves data gated to that vendor's declared capabilities, no real backend. Switchable at runtime via `POST /api/dev-mode/broker`. Inert in production. |
| `DEV_BROKER_FAULTS` | gateway | **Dev only.** Make selected broker calls fail, so degraded-read behaviour can be exercised without a real outage: `method[:arg][@timeout]`, comma-separated (e.g. `projectFinancials:p-2`, `listProjects@timeout`). Read only under dev mode and inert under `NODE_ENV=production`; a malformed value degrades to no faults rather than failing everything. See [docs/DEGRADED-READS.md](docs/DEGRADED-READS.md). |
...
| `PORTFOLIO_FANOUT_LIMIT` | gateway | Concurrency of the per-project portfolio fan-out (default `10`, max `200`). Only applies when the broker does NOT implement the bulk `portfolioFinancials`/`portfolioCapacity` reads; a bulk-capable broker makes one call and ignores this. Raising it shortens a portfolio view linearly and risks backend 429s — measure first. |
| `PORTFOLIO_FANOUT_MAX_PROJECTS` | gateway | Ceiling on that fallback fan-out (default `500`). Above it the roll-up totals are refused rather than attempted and reported as unavailable, because the fan-out is O(projects): ~15s at 3,000 projects against a 50ms backend, ~150s at 30,000. The fix is a bulk-capable broker, not a higher ceiling. See [docs/DEGRADED-READS.md](docs/DEGRADED-READS.md). |
...
New to the codebase, or auditing it? Start with these three. Each is designed to
be read in one sitting and lets you *see* how the system works (architecture, the
...
- **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)** covers the system overview: the
...
- **[docs/READING-GUIDE.md](docs/READING-GUIDE.md)** answers "where do I look to
...
- **For enterprise buyers → [docs/ENTERPRISE-READINESS.md](docs/ENTERPRISE-READINESS.md)**:
  a buyer-panel gap analysis across CEO, Finance, Compliance, CISO, IT and Projects,
  covering what OmniProject already delivers per seat, the gaps to close, and a
  prioritised enterprise-readiness roadmap.
...
OmniProject is pre-1.0. Maturity varies by feature, so each one is tagged and a
preview is never mistaken for a production guarantee:
...
- **Beta** (functional and tested, but new and not yet hardened by real use):
  [Exploration mode](docs/EXPLORATION.md), covering snapshots → trends, auto-snapshot
...
**Open-core.** The core is **Apache-2.0** ([`LICENSE`](LICENSE)) and free for any
...
enforcement is dormant, deliberately and temporarily. See **[LICENSING.md](LICENSING.md)**
...
enterprise integrations: **the payment buys you the time saved, and the permission
is already yours under Apache-2.0.** If you'd rather not build it yourself, we can
build a view or workflow for you as an **optional paid service**, selling our time
and delivering ordinary open source you own. See
...
> integration gives you a **head-start rather than a fix-it guarantee**. **Paid
> support / maintained-tier packages are planned** as a separate offering as the
