...
The positioning, ready-to-paste posts, the GitHub release note, and the
pre-launch checklist. Swap in the repo URL where noted. Update this as the
messaging changes.
...
> (`0.1.0`, `0.2.0`, … with no `v`, and one stray historical exception).
...
1. **Programme management, above the task board.** Finance (EVM/CPI/SPI,
   multi-currency), time/schedule (Gantt, milestones), and resource
   capacity/allocation.
...
3. **One neutral broker, with n8n as the reference default.** No hand-rolled
...
5. **Fits what you already run.** Your tools, your broker, your SSO stay put;
   adopt the views and connections you want, ignore the rest.
6. **Speaks your methodology.** Kanban, Scrum, Gantt/Waterfall, PRINCE2, RAID and
   list over one dataset, switched per team.
7. **Safe to try with real data.** It stores nothing, and *you* control whether
   ...
   read-only. Dry-run verify, sandbox, and one-click rollback. That answers the #1
   objection for any new tool ("I'm not letting it write to my Jira") up front.

### What's genuinely differentiated (be honest)

The *category* (PM aggregator / single pane of glass) is crowded. The
*architecture* is not: **stateless + broker-agnostic, with n8n as the reference
broker**, and no direct competitor builds it this way. Lead with the architecture
rather than "single pane of glass." The honest gap to close at launch is
**validation**: get one real team running it against a real backend; that matters
more than any next feature.
...
| **r/n8n** | Perfect niche, since n8n is the reference broker | Go technical; show the contract + blueprints. |
...
| **r/projectmanagement** | Your end users | Strict on self-promo; read the rules, and expect to need a flair or mod approval. Frame as "a free tool for X". |
...
right flair; Tue–Thu ~8–10am US Eastern lands best; reply to everything, including
criticism.
...
>
> It's **open-core**: **Apache-2.0** for everything above, with a few enterprise
> extras (white-labeling, outbound webhooks, SAP/Primavera workflow generators)
> behind a licence key. **Pre-1.0, provided as-is (no warranty).** I'm launching
> to get real feedback and start a community.
>
> Repo: `<your link>`. I'd love your thoughts on the
...
> with a small licensed-feature tier. Pre-1.0, no warranty. Feedback welcome,
> above all on the stateless, broker-agnostic architecture (n8n as the reference broker).
>
...
Paste as the Release body when tagging `0.1.0`. (A template; adapt per release.)
...
- [ ] Open the **🧪 tester call-out**, either the pinned Discussion this section
      implies or an issue from the `beta_tester.yml` template, so every
