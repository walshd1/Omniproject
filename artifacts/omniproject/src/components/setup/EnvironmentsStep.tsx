...
export function EnvironmentsStep({ isAdmin }: { isAdmin: boolean }) {
  ...
  const loadEnvironments = () => {
    setLoadError(false);
    fetchEnvironments().then(setStore).catch(() => setLoadError(true));
  };
  ...
  return (
    ...
    <Step n={7} title="Environments & rollback">
      ...
      {!isAdmin ? (
        ...
        <DataState isLoading={!store && !loadError} isError={loadError} onRetry={loadEnvironments} error="Couldn't load environments." className="w-full">
          {store && (
            <>
