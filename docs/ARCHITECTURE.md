...
OmniProject versions its own **configuration** (never project data). Four tiers:
...
| **Scope-layered config defs** | [`lib/scoped-config.ts`](../artifacts/api-server/src/lib/scoped-config.ts) + the def store | The **composition-model** tier: settings that resolve **system < org < programme < project < user** as `config` **def** artifacts, folded by `resolveScopedConfig`. Most former `SettingsState` keys now live here (scheduling, accessibility, branding/labels, methodology composition, the register/forms/screens/reports collections, and the Phase C security keys `error-telemetry`, `logging-sync`, `history-retention`, `self-host`). **Security-classified** configs relax only through the **floor gate** (a relaxing write → sign-off proposal); the AI provider/model/STT **allowlist floors** resolve so a lower scope may only *tighten*. Rides the DEF half of the full backup. |
...
| **Settings store** | [`lib/settings.ts`](../artifacts/api-server/src/lib/settings.ts) + [`lib/config-store.ts`](../artifacts/api-server/src/lib/config-store.ts) | Gateway-local runtime config for the keys not yet on the composition tier: broker URL, backend routing hint, role-map, field overrides, and the security machinery (trust roots, webhooks, federated peers, approval chains, governance rules). **In-memory (per-replica) by default** — a runtime settings change is not shared across replicas and, without persistence, does not survive a restart. Set `CONFIG_STORE_FILE` to persist environments + version history (retention `CONFIG_STORE_MAX_VERSIONS`, default 100); the persisted **active environment is re-applied to live settings at boot** so a runtime config change survives a restart, and **rollback** history is available. The "stateless / zero-at-rest" guarantee is about **project data** — this gateway *config* is deliberately gateway-local state; back it with `CONFIG_STORE_FILE` (durability) and a shared config source for multi-replica agreement (see [ops/MULTI-REPLICA.md](ops/MULTI-REPLICA.md)). |
