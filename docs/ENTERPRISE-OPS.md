...
- **Identity:** a subject's identity record lives in your IdP; deprovision via SCIM
  (`active=false`) or the IdP directly — that denies access here too (immediate on the handling replica; fleet-wide within the sync interval via shared state when `REDIS_URL` is set — see `docs/ops/MULTI-REPLICA.md`).
...
| Deprovision a user | SCIM `active=false` / IdP (denies login immediately on the handling replica; fleet-wide within the sync interval via shared state when `REDIS_URL` is set) |
