...
export function ListView({ projectId }: { projectId: string }) {
  ...
  const { statusOrder, priorityOrder, statusLabel, priorityLabel } = useWorkVocabulary();
  ...
  const rows = useMemo(() => {
    ...
    const rank = (i: Issue): number | string => {
      switch (sort.key) {
        case "status": return statusOrder.indexOf(i.status);
        case "priority": return priorityOrder.indexOf(i.priority);
    ...
    return arr;
  }, [issues, sort, statusOrder, priorityOrder]);
          ...
          <tbody>
            ...
            {visibleRows.map((issue) => (
              ...
              >
                ...
                <td className="px-3 py-2"><span className="inline-flex items-center gap-1.5"><StatusDot status={issue.status} />{statusLabel(issue.status)}</span></td>
                <td className="px-3 py-2"><span className="inline-flex items-center gap-1.5"><PriorityDot priority={issue.priority} />{priorityLabel(issue.priority)}</span></td>
