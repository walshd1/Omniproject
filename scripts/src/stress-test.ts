...
async function main() {
  ...
  async function worker() {
    while (true) {
      ...
      const t0 = Date.now();
      try {
        const r = await fetch(`${base}${plan[i]}`, { headers });
  ...
  console.log(`  latency p50:  ${pct(latencies, 50)} ms`);
  console.log(`  latency p95:  ${pct(latencies, 95)} ms`);
  console.log(`  latency p99:  ${pct(latencies, 99)} ms`);
