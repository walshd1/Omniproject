...
### `lib/backend-catalogue/src/finance-consolidation.ts`

Portfolio financial consolidation — the ONE pure, dependency-free implementation shared by the SPA (the Portfolio Financials report + Exec Board Pack) and the gateway (the `/api/portfolio/financials` fan-out).
