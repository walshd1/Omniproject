 ...
 * ARTIFACTS in the encrypted def store, authored through the ONE importer; the ENGINE (the CustomReport
 * renderer) reads them here. Precedence, lowest → highest:
 *   org  <  project  <  user
 * so a customer copy overrides an older one of the same id. (Overrides of the SHIPPED built-in reports stay in
 * the separate `reportOverrides` settings overlay — metadata tweaks, not standalone defs.)
 */
