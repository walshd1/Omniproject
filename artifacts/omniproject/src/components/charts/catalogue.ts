...
/**
 * The primitive catalogue — a data-only library of every rendering primitive the app ships, so users
 * (and the view/report/chart builders) can discover what they can compose their own artifacts from.
 *
 * This is metadata, not components: each entry names a primitive, the shape of data it consumes, and the
 * options it accepts. A builder palette renders straight off this array; nothing here imports React, so
 * it is safe to serialise into a definition file. The `chartType` field, when present, is the
 * `ChartViewSpec["type"]` the primitive is reached through in the common ChartView renderer — that is the
 * single source of truth binding a catalogue entry to what actually draws it.
 */
export type PrimitiveCategory = "chart" | "graphic" | "table" | "tile";

/** The kind of value a parameter carries — a builder maps these to the right input control / data picker. */
export type PrimitiveParamType =
  | "rows" // ChartRow[] — tabular data, one object per row
  | "series" // ChartSeries[] — which row keys to plot and how to label them
  | "slices" // { name, value }[] — share/proportion data
  | "points" // ScatterPoint[] / (number|null)[] — plotted points
  | "tree" // TreeNode[] — hierarchy for treemap / work breakdown
  | "items" // GanttItem[] — dated bars
...
export interface PrimitiveDef {
  ...
  description: string;
  /** The ChartView spec type this primitive draws through, when it is a ChartView-dispatchable chart. */
  chartType?: ChartViewSpec["type"];
