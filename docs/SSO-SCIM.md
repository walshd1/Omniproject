...
3. OmniProject (immediate on the handling replica; fleet-wide within the sync interval via shared state when `REDIS_URL` is set — see `docs/ops/MULTI-REPLICA.md`) denies the user at the gate (even with an unexpired session/token),
