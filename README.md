...
| `BROKER_SPOOF` | gateway | **Dev only.** A vendor id (e.g. `openproject`) to present AS — the gateway serves contract-compliant demo data gated to that vendor's declared capabilities, with no real backend. For testing capability gating / screens / replay against a vendor profile when the backend is unreachable. Inert in production. |
