/**
 * SCOPE-OVERRIDABLE RAID/risk IMPACT vocabulary — the resolver + write sanitiser behind
 * `GET`/`PUT /api/impact-vocabulary`. The shipped default (assets/impact-vocabulary.json, seeded as the
 * system-scope `impact-vocabulary` config def) is the base; org/programme/project/user layers fold on top
 * via the shared `resolveConfig` (nearest scope wins, id-keyed arrays merge by id), exactly like the severity
 * vocabulary next door — and like `scheduling`.
 *
 * This is the IMPACT axis of a RAID/risk entry (the consequence magnitude — the I in risk-exposure P×I).
 * GRADES are fully org-owned: a scope may RELABEL, REORDER, ADD and REMOVE impact grades and tag them by
 * methodology. The ONE invariant kept for the risk-exposure maths: every effective grade must still declare an
 * internal ordinal `level` — a custom grade without one is dropped, and the sanitiser requires it on a
 * newly-added grade. That ordinal is what {@link resolveRiskExposure} keys off (snapped onto the nearest
 ...
 */
import { impactVocabularyValues, type ResolvedImpact, type ImpactVocabularyValues } from "@workspace/backend-catalogue";
import { configDefLayers, resolveScopedConfig, type ConfigScopes } from "./scoped-config";
import { makeScopedId } from "./artifact-store";

export const IMPACT_VOCABULARY_CONFIG_ID = "impact-vocabulary";
/** The singleton org-scope override row id (stable, so a save upserts rather than piling rows). */
export const ORG_IMPACT_VOCABULARY_ID = makeScopedId("org", `config-${IMPACT_VOCABULARY_CONFIG_ID}`);
...
/**
 * The effective RAID/risk impact vocabulary at the given scopes: the shipped default with every
 * `impact-vocabulary` config-def layer folded on top (system → org → programme → project → user), nearest
 ...
 */
export function resolveImpactVocabulary(scopes: ConfigScopes = {}): ImpactVocabularyValues {
  const layers = configDefLayers(IMPACT_VOCABULARY_CONFIG_ID, scopes);
  const folded = resolveScopedConfig<Record<string, unknown>>(impactVocabularyValues() as unknown as Record<string, unknown>, layers);
  return { levels: projectLevels(folded["levels"]) };
}

/** Project a folded impact-grade array: keep only well-formed, non-tombstoned entries (a valid grade needs a
...
function projectLevels(folded: unknown): ResolvedImpact[] {
  const arr = Array.isArray(folded) ? folded : [];
  const out: ResolvedImpact[] = [];
...
/** One sanitised impact-grade override entry — a partial for an existing grade, a full def for a new one, or
 *  a `{id, removed}` tombstone. */
export interface ImpactGradeOverride { id: string; label?: string; labels?: Record<string, string>; order?: number; level?: number; methodologies?: string[]; color?: string; removed?: true }
 ...
 * dropped.
 */
export function sanitizeImpactVocabularyOverride(raw: unknown): { levels: ImpactGradeOverride[] } {
  if (typeof raw !== "object" || raw === null || Array.isArray(raw)) throw new Error("impact vocabulary override must be an object");
  const base = impactVocabularyValues();
  const obj = raw as Record<string, unknown>;
  return { levels: cleanLevelOverrides(obj["levels"], new Set(base.levels.map((l) => l.id))) };
}

function cleanLevelOverrides(list: unknown, baseIds: Set<string>): ImpactGradeOverride[] {
  if (list === undefined) return [];
  if (!Array.isArray(list)) throw new Error("levels must be an array");
  const out: ImpactGradeOverride[] = [];
  for (const raw of list) {
    ...
    const entry: ImpactGradeOverride = { id };
