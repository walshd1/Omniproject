...
// The catalogue-freeze policy (see lib/backend-catalogue/vendors/README.md) —
// fail loudly, before touching the generated files, if the backends plane has
// grown past baseline without the flagship set verified.
const backendsLoaded = loaded.find((l) => l.group.label === "backends");
if (backendsLoaded) checkCatalogueFreeze(backendsLoaded.rows as { id: string; verification?: string }[]);
