...
- **Deployment:** `Dockerfile` (builds the single `omni-shell` image), `docker-compose.standalone.yml`, `docker-compose.enterprise.yml` (full BYO-SSO + backends + AI base), `docker-compose.slim.yml` (lean stack), `docker-compose.loadtest.yml`, `k8s-enterprise-manifest.yaml`.
...
- **Single-container "omni-shell".** The deploy artifacts run one image on port 3000. The Express server serves both `/api/*` and the built SPA (`STATIC_DIR`) with a history fallback, so the SPA and gateway share an origin in production. In dev they're split (Vite proxies `/api`).
