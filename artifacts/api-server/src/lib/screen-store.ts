 ...
 * in the encrypted def store; the SPA merges these over its built-in catalogue (an override wins by id, or adds
 * a net-new screen). The ENGINE (ScreenRenderer + panels) stays code. Precedence, lowest → highest:
 *   org  <  project  <  user
 * so a customer copy overrides an older one of the same id. The shipped `system` catalogue is deliberately
 * EXCLUDED here: the SPA already carries the built-ins (the same relocated catalogue), so this endpoint returns
 * only the customer OVERRIDES to merge on top.
 */
