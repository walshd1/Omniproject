...
test("backends: an ISSUE backend must expose projects + issues; an INVOICE backend must expose its invoice list", () => {
  ...
  for (const [rec, read] of [["vendor", "list_vendors"], ["expense", "list_expenses"], ["bill", "list_bills"], ["purchase_order", "list_purchase_orders"], ["gl_account", "list_accounts"], ["journal_entry", "list_journal_entries"], ["bank_account", "list_bank_accounts"], ["bank_transaction", "list_bank_transactions"], ["fx_rate", "list_fx_rates"], ["fiscal_period", "list_fiscal_periods"], ["dimension", "list_dimensions"], ["rev_rec_schedule", "list_rev_rec_schedules"], ["goods_receipt", "list_goods_receipts"], ["fixed_asset", "list_fixed_assets"]] as const) {
