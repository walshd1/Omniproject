/**
 * SCOPE-OVERRIDABLE RAID/risk LIKELIHOOD vocabulary — the resolver + write sanitiser behind
 * `GET`/`PUT /api/likelihood-vocabulary`. The shipped default (assets/likelihood-vocabulary.json, seeded as
 * the system-scope `likelihood-vocabulary` config def) is the base; org/programme/project/user layers fold on
 * top via the shared `resolveConfig` (nearest scope wins, id-keyed arrays merge by id), exactly like the
 * impact vocabulary next door — and like `scheduling`.
 *
 * This is the LIKELIHOOD axis of a RAID/risk entry (the probability a risk occurs — the P in risk-exposure
 * P×I). GRADES are fully org-owned: a scope may RELABEL, REORDER, ADD and REMOVE likelihood grades and tag
 * them by methodology. The ONE invariant kept for the risk-exposure maths: every effective grade must still
 * declare an internal ordinal `level` — a custom grade without one is dropped, and the sanitiser requires it
 * on a newly-added grade. That ordinal is what {@link resolveRiskExposure} keys off (snapped onto the nearest
 * shipped band, so a scope-added grade still yields a bounded number). Removal is a tombstone (`{id,
 * removed:true}`) folded over the base.
 */
import { likelihoodVocabularyValues, type ResolvedLikelihood, type LikelihoodVocabularyValues } from "@workspace/backend-catalogue";
import { configDefLayers, resolveScopedConfig, type ConfigScopes } from "./scoped-config";
import { makeScopedId } from "./artifact-store";

export const LIKELIHOOD_VOCABULARY_CONFIG_ID = "likelihood-vocabulary";
/** The singleton org-scope override row id (stable, so a save upserts rather than piling rows). */
export const ORG_LIKELIHOOD_VOCABULARY_ID = makeScopedId("org", `config-${LIKELIHOOD_VOCABULARY_CONFIG_ID}`);
...
/**
 * The effective RAID/risk likelihood vocabulary at the given scopes: the shipped default with every
 * `likelihood-vocabulary` config-def layer folded on top (system → org → programme → project → user), nearest
 ...
 */
export function resolveLikelihoodVocabulary(scopes: ConfigScopes = {}): LikelihoodVocabularyValues {
  const layers = configDefLayers(LIKELIHOOD_VOCABULARY_CONFIG_ID, scopes);
  const folded = resolveScopedConfig<Record<string, unknown>>(likelihoodVocabularyValues() as unknown as Record<string, unknown>, layers);
  return { levels: projectLevels(folded["levels"]) };
}

/** Project a folded likelihood-grade array: keep only well-formed, non-tombstoned entries (a valid grade needs
 *  a label + an order + an ordinal `level`), dedupe by id, default methodology tags, sort by order. Add/remove
 *  are honoured — the set is whatever the folded layers say, not a fixed list. */
function projectLevels(folded: unknown): ResolvedLikelihood[] {
  const arr = Array.isArray(folded) ? folded : [];
  const out: ResolvedLikelihood[] = [];
...
/** One sanitised likelihood-grade override entry — a partial for an existing grade, a full def for a new one,
 *  or a `{id, removed}` tombstone. */
export interface LikelihoodGradeOverride { id: string; label?: string; labels?: Record<string, string>; order?: number; level?: number; methodologies?: string[]; color?: string; removed?: true }
 ...
 * dropped.
 */
export function sanitizeLikelihoodVocabularyOverride(raw: unknown): { levels: LikelihoodGradeOverride[] } {
  if (typeof raw !== "object" || raw === null || Array.isArray(raw)) throw new Error("likelihood vocabulary override must be an object");
  const base = likelihoodVocabularyValues();
  const obj = raw as Record<string, unknown>;
  return { levels: cleanLevelOverrides(obj["levels"], new Set(base.levels.map((l) => l.id))) };
}

function cleanLevelOverrides(list: unknown, baseIds: Set<string>): LikelihoodGradeOverride[] {
  if (list === undefined) return [];
  if (!Array.isArray(list)) throw new Error("levels must be an array");
  const out: LikelihoodGradeOverride[] = [];
  for (const raw of list) {
    ...
    const entry: LikelihoodGradeOverride = { id };
